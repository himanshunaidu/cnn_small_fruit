...
#Take in RGB images
convo1 = convolutional_layer(x_image, shape=[3, 3, 3, 64], stridec=[1, 1, 1, 1])
convo1_relu = tf.nn.relu(convo1)
#lrn1 = LRN(convo1, 2, 2e-05, 0.75, "norm1")
convo1_pool = max_pool_2by2(convo1_relu)

convo2 = convolutional_layer(convo1_pool, shape=[3, 3, 64, 64])
convo2_relu = tf.nn.relu(convo2)
#lrn2 = LRN(convo2, 2, 2e-05, 0.75, "norm1")
convo2_pool = max_pool_2by2(convo2_relu)

#DROPOUT 1
pool_prob = tf.placeholder(tf.float32)
convo2_pool_drop = tf.nn.dropout(convo2_pool, keep_prob=pool_prob)

print('Convolutional layers end')

#the dimension  might be wrong because of insufficient details of strides and paddings of some layers

convo2_flat = tf.reshape(convo2_pool_drop, [-1, 25*25*64])
full_layer_one = tf.nn.relu(normal_full_layer(convo2_flat, 500))
...
var_export_dir = '<PATH>/TempF5/variables.ckpt'
...
step_saver = open('<PATH>/TempF5/steps.txt', 'a+')
acc_saver = open('<PATH>/TempF5/acc.txt', 'a+')
...
with tf.Session() as sess:
    ...
    for i in range((actual_steps-runs), steps):
        ...
        train_time_saver = open('<PATH>/TempF5/train_time.txt', 'a+')
        ...
        train_time_saver.write(str(step_time)+'\n')
        train_time_saver.close()
